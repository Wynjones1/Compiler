...
#endif
	g_token_count++;
	*token = realloc(*token, sizeof(token_t) * (g_token_count + 1));
...
static token_t *read_id(char **pos_, token_t *in)
{
	...
	while( (c = *pos++) )
	{
		if(isseperator(c))
		{
			char *string = calloc(1, (size_t)(pos - *pos));
...
static token_t *read_string(char **pos_, token_t *in)
{
	...
	*pos_ = pos;
	...
	in[g_token_count].string        = malloc( count + 1);
